...
var Transaction = lbchcore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/lbch.conf')));
var LBChService = proxyquire('../../lib/services/lbchd', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultLBChConf = fs.readFileSync(path.resolve(__dirname, '../data/default.lbch.conf'), 'utf8');

describe('LBCh Service', function() {
	...
	var baseConfig = {
		node: {
			network: lbchcore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var lbchd = new LBChService(baseConfig);
			should.exist(lbchd);
		});
		it('will create an instance without `new`', function() {
			var lbchd = LBChService(baseConfig);
			should.exist(lbchd);
		});
		it('will init caches', function() {
			var lbchd = new LBChService(baseConfig);

			should.exist(lbchd.utxosCache);
			should.exist(lbchd.txidsCache);
			should.exist(lbchd.balanceCache);
			should.exist(lbchd.summaryCache);
			should.exist(lbchd.transactionDetailedCache);
			should.exist(lbchd.accountInfo);

			should.exist(lbchd.transactionCache);
			should.exist(lbchd.rawTransactionCache);
			should.exist(lbchd.rawJsonTransactionCache);
			should.exist(lbchd.blockCache);
			should.exist(lbchd.blockJsonCache);
			should.exist(lbchd.blockSubsidyCache);
			should.exist(lbchd.rawBlockCache);
			should.exist(lbchd.blockHeaderCache);
			should.exist(lbchd.zmqKnownTransactions);
			should.exist(lbchd.zmqKnownBlocks);
			should.exist(lbchd.lastTip);
			should.exist(lbchd.lastTipTimeout);
			should.equal(lbchd.dgpInfoCache, null, 'should be null');
			should.equal(lbchd.miningInfoCache, null, 'should be null');
			should.equal(lbchd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(lbchd.maxTxids, 1000);
			should.equal(lbchd.maxTransactionHistory, 50);
			should.equal(lbchd.maxAddressesQuery, 10000);
			should.equal(lbchd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(lbchd.spawnRestartTime, 5000);
			should.equal(lbchd.spawnStopTime, 10000);

			// try all interval
			should.equal(lbchd.tryAllInterval, 1000);
			should.equal(lbchd.startRetryInterval, 5000);

			// rpc limits
			should.equal(lbchd.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(lbchd.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var lbchd = new LBChService(baseConfig);
			lbchd.nodes.should.deep.equal([]);
			lbchd.nodesIndex.should.equal(0);
			lbchd.nodes.push({ client: sinon.stub() });
			should.exist(lbchd.client);
		});
		it('will set subscriptions', function() {
			var lbchd = new LBChService(baseConfig);
			lbchd.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var lbchd = new LBChService(baseConfig);
			lbchd._initDefaults({ transactionConcurrency: 10 });
			lbchd.transactionConcurrency.should.equal(10);
			lbchd._initDefaults({});
			lbchd.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			LBChService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var lbchd = new LBChService(baseConfig);
			var methods = lbchd.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var lbchd = new LBChService(baseConfig);
			var events = lbchd.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('lbchd/rawtransaction');
			events[0].scope.should.equal(lbchd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('lbchd/hashblock');
			events[1].scope.should.equal(lbchd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('lbchd/addresstxid');
			events[2].scope.should.equal(lbchd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('lbchd/addressbalance');
			events[3].scope.should.equal(lbchd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var lbchd = new LBChService(baseConfig);
			lbchd.subscribe = sinon.stub();
			lbchd.unsubscribe = sinon.stub();
			var events = lbchd.getPublishEvents();

			events[0].subscribe('test');
			lbchd.subscribe.args[0][0].should.equal('rawtransaction');
			lbchd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			lbchd.unsubscribe.args[0][0].should.equal('rawtransaction');
			lbchd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			lbchd.subscribe.args[1][0].should.equal('hashblock');
			lbchd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			lbchd.unsubscribe.args[1][0].should.equal('hashblock');
			lbchd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var lbchd = new LBChService(baseConfig);
			var emitter = {};
			lbchd.subscribe('hashblock', emitter);
			lbchd.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			lbchd.subscribe('rawtransaction', emitter2);
			lbchd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			lbchd.subscribe('hashblock', emitter1);
			lbchd.subscribe('hashblock', emitter2);
			lbchd.subscribe('hashblock', emitter3);
			lbchd.subscribe('hashblock', emitter4);
			lbchd.subscribe('hashblock', emitter5);
			lbchd.subscriptions.hashblock.length.should.equal(5);

			lbchd.unsubscribe('hashblock', emitter3);
			lbchd.subscriptions.hashblock.length.should.equal(4);
			lbchd.subscriptions.hashblock[0].should.equal(emitter1);
			lbchd.subscriptions.hashblock[1].should.equal(emitter2);
			lbchd.subscriptions.hashblock[2].should.equal(emitter4);
			lbchd.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var lbchd = new LBChService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			lbchd.subscriptions.hashblock = [emitter1];
			lbchd.unsubscribe('hashblock', emitter3);
			lbchd.subscriptions.hashblock.length.should.equal(1);
			lbchd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			lbchd.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(lbchd.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var lbchd = new LBChService(baseConfig);
			var emitter = new EventEmitter();
			lbchd.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var lbchd = new LBChService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			lbchd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			lbchd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			lbchd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.unsubscribeAddress(emitter1);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			lbchd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(lbchd, 'unsubscribeAddressAll');
			lbchd.unsubscribeAddress(emitter1);
			lbchd.unsubscribeAddressAll.callCount.should.equal(1);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			lbchd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			lbchd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			lbchd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			lbchd.unsubscribeAddressAll(emitter1);
			lbchd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			lbchd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			lbchd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(lbchd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var lbchd = new LBChService(baseConfig);
			var config = lbchd._getDefaultConfig();
			config.should.equal(defaultLBChConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a lbch.conf file', function() {
			var TestLBCh = proxyquire('../../lib/services/lbchd', {
				fs: {
					readFileSync: readFileSync,
			...
			var lbchd = new TestLBCh(baseConfig);
			lbchd.options.spawn.datadir = '/tmp/.lbch';

			sinon.spy(lbchd, '_expandRelativeDatadir');
			sinon.spy(lbchd, '_checkConfigIndexes');
			sinon.spy(lbchd, '_getDefaultConf')
			sinon.spy(lbchd, '_getNetworkConfigPath');
			sinon.spy(lbchd, '_parseBitcoinConf');

			var node = {};
			lbchd._loadSpawnConfiguration(node);

			lbchd._expandRelativeDatadir.callCount.should.equal(1);
			lbchd._checkConfigIndexes.callCount.should.equal(1);
			lbchd._getDefaultConf.callCount.should.equal(1);
			lbchd._getNetworkConfigPath.callCount.should.equal(1);
			lbchd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(lbchd.spawn.config);
			lbchd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: lbchcore.Networks.testnet,
					configPath: '/tmp/.lbchcore/lbchcore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			lbchd.options.spawn.datadir = './data';
			var node = {};
			lbchd._loadSpawnConfiguration(node);
			...
			lbchd.options.spawn.datadir.should.equal('/tmp/.lbchcore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestLBCh = proxyquire('../../lib/services/lbchd', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badlbch.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var lbchd = new TestLBCh(baseConfig);
			(function() {
				lbchd._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(lbchcore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the lbch.conf file', function() {
			var lbchd = new LBChService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				lbchd._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the lbch.conf file', function() {
			var lbchd = new LBChService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the lbch.conf file', function() {
			var lbchd = new LBChService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the lbch.conf file', function() {
			var lbchd = new LBChService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the lbch.conf file', function() {
			var lbchd = new LBChService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				lbchd.transactionDetailedCache.set(keys[i], {});
				lbchd.utxosCache.set(keys[i], {});
				lbchd.accountInfo.set(keys[i], {});
				lbchd.txidsCache.set(keys[i], {});
				lbchd.balanceCache.set(keys[i], {});
				lbchd.summaryCache.set(keys[i], {});
				lbchd.blockOverviewCache.set(keys[i], {});
				lbchd.blockJsonCache.set(keys[i], {});
			}
			lbchd._resetCaches();
			should.equal(lbchd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(lbchd.utxosCache.get(keys[0]), undefined);
			should.equal(lbchd.accountInfo.get(keys[0]), undefined);
			should.equal(lbchd.txidsCache.get(keys[0]), undefined);
			should.equal(lbchd.balanceCache.get(keys[0]), undefined);
			should.equal(lbchd.summaryCache.get(keys[0]), undefined);
			should.equal(lbchd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(lbchd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			lbchd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				lbchd.nodes[0].client.getInfo.callCount.should.equal(1);
				lbchd.nodes[1].client.getInfo.callCount.should.equal(1);
				lbchd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			lbchd._tryAllClients(function(client, next) {
				...
				lbchd.nodes[1].client.getInfo.callCount.should.equal(1);
				lbchd.nodes[2].client.getInfo.callCount.should.equal(1);
				lbchd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert lbchd-rpc error object into JavaScript error', function() {
			var lbchd = new LBChService(baseConfig);
			var error = lbchd._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			lbchd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			lbchd.nodes.push({
				client: {
			...
			lbchd._initChain(function() {
				log.info.callCount.should.equal(1);
				lbchd.getRawBlock.callCount.should.equal(1);
				lbchd.getRawBlock.args[0][0].should.equal('genesishash');
				lbchd.height.should.equal(5000);
				lbchd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			lbchd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			lbchd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			lbchcore.Networks.disableRegtest();
			baseConfig.node.network = lbchcore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: lbchcore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var lbchd = new LBChService(config);
			lbchd._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var lbchd = new LBChService(config);
			lbchd._getDefaultConf().rpcport.should.equal(18332);
		});
		it('will get default rpc port for regtest', function() {
			lbchcore.Networks.enableRegtest();
			var config = {
				node: {
					network: lbchcore.Networks.testnet
				},
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var lbchd = new LBChService(config);
			should.equal(lbchd._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var lbchd = new LBChService(config);
			lbchd._getNetworkConfigPath().should.equal('testnet3/lbch.conf');
		});
		it('will get default rpc port for regtest', function() {
			lbchcore.Networks.enableRegtest();
			var config = {
			...
			var lbchd = new LBChService(config);
			lbchd._getNetworkConfigPath().should.equal('regtest/lbch.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var lbchd = new LBChService(baseConfig);
			lbchd.node.network = lbchcore.Networks.testnet;
			lbchd._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var lbchd = new LBChService(baseConfig);
			lbchd.node.network = lbchcore.Networks.testnet;
			lbchcore.Networks.enableRegtest();
			lbchd._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var lbchd = new LBChService(baseConfig);
			lbchd.node.network = lbchcore.Networks.livenet;
			lbchcore.Networks.enableRegtest();
			should.equal(lbchd._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			lbchd._rapidProtectedUpdateTip = sinon.stub();
			lbchd.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			lbchd._zmqBlockHandler(node, message);
			lbchd._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			lbchd._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(lbchd, 'emit');
			lbchd.on('block', function(block) {
				block.should.equal(message);
				lbchd.emit.callCount.should.equal(1);
				lbchd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			lbchd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			lbchd._zmqBlockHandler(node, message);
			lbchd._rapidProtectedUpdateTip.callCount.should.equal(1);
			lbchd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			lbchd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(lbchd, 'emit');
			var emitter = new EventEmitter();
			lbchd.subscriptions.hashblock.push(emitter);
			emitter.on('lbchd/hashblock', function(blockHash) {
				lbchd._rapidProtectedUpdateTip.callCount.should.equal(1);
				lbchd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				lbchd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.syncPercentage = sinon.stub();
			sinon.spy(lbchd, '_resetCaches');
			lbchd.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				lbchd._resetCaches.callCount.should.equal(1);
			...
			lbchd._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			lbchd.on('synced', function() {
				lbchd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			lbchd.on('synced', function() {
				lbchd.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			lbchd.on('error', function(err) {
				log.error.callCount.should.equal(1);
				lbchd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.syncPercentage = sinon.stub();
			lbchd._resetCaches = sinon.stub();
			lbchd.on('tip', function(height) {
				lbchd._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				lbchd.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using lbchcore.Transaction', function() {
			var lbchd = new LBChService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = lbchcore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(lbchcore.Networks.testnet);
			var outputAddress = lbchcore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = lbchcore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: lbchcore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = lbchd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(lbchcore.Transaction.Output({
				script: lbchcore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = lbchd._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var lbchd = new LBChService(baseConfig);
			var tx = lbchcore.Transaction();
			var address = lbchcore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(lbchcore.Transaction.Output({
				script: lbchcore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(lbchcore.Transaction.Output({
				script: lbchcore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			lbchd._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			lbchd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('lbchd/addresstxid', function(data) {
			...
			lbchd._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			lbchd.subscriptions.rawtransaction.push(emitter);
			emitter.on('lbchd/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			lbchd._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(lbchd, 'emit');
			lbchd.subscriptions.rawtransaction.push(emitter);
			emitter.on('lbchd/rawtransaction', function() {
				done();
			});
			var node = {};
			lbchd._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd._updateTip = sinon.stub();
			lbchd._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			lbchd.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			lbchd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				lbchd._updateTip.callCount.should.equal(11);
				lbchd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			lbchd._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				lbchd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			lbchd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				lbchd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			lbchd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				lbchd._zmqBlockHandler.callCount.should.equal(0);
				lbchd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var lbchd = new LBChService(baseConfig);
			var node = {};
			lbchd._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			lbchd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			lbchd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			lbchd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestLBChService = proxyquire('../../lib/services/lbchd', {
				fs: {
					readFile: readFile
				}
			});
			var lbchd = new TestLBChService(baseConfig);
			lbchd.spawnStopTime = 1;
			lbchd._process = {};
			lbchd._process.kill = sinon.stub();
			lbchd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				lbchd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			lbchd._process.kill = sinon.stub().throws(error2);
			lbchd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd._loadSpawnConfiguration = sinon.stub();
			lbchd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			lbchd._spawnChildProcess(function(err) {
				lbchd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var lbchd = new LBChService(baseConfig);
			lbchd._loadSpawnConfiguration = sinon.stub();
			lbchd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			lbchd._spawnChildProcess(function(err) {
				lbchd._loadSpawnConfiguration.callCount.should.equal(1);
				lbchd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var lbchd = new TestLBChService(config);
			lbchd.spawn = {};
			lbchd._loadSpawnConfiguration = sinon.stub();
			lbchd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			lbchd.node.stopping = true;
			lbchd._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			lbchd._loadSpawnConfiguration = sinon.stub();
			lbchd.spawn = {};
			lbchd.spawn.exec = 'testexec';
			lbchd.spawn.configPath = 'testdir/lbch.conf';
			lbchd.spawn.datadir = 'testdir';
			lbchd.spawn.config = {};
			lbchd.spawn.config.rpcport = 20001;
			lbchd.spawn.config.rpcuser = 'lbch';
			lbchd.spawn.config.rpcpassword = 'password';
			lbchd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			lbchd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			lbchd._initZmqSubSocket = sinon.stub();
			lbchd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			lbchd._checkReindex = sinon.stub().callsArgWith(1, null);
			lbchd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/lbch.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				lbchd._loadTipFromNode.callCount.should.equal(1);
				lbchd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(lbchd._initZmqSubSocket.args[0][0].client);
				lbchd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				lbchd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(lbchd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn lbchd spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestLBChService = proxyquire('../../lib/services/lbchd', {
				fs: {
			...
			lbchd._loadSpawnConfiguration = sinon.stub();
			lbchd.spawn = {};
			lbchd.spawn.exec = 'lbchd';
			lbchd.spawn.datadir = '/tmp/lbch';
			lbchd.spawn.configPath = '/tmp/lbch/lbch.conf';
			lbchd.spawn.config = {};
			lbchd.spawnRestartTime = 1;
			lbchd._loadTipFromNode = sinon.stub().callsArg(1);
			lbchd._initZmqSubSocket = sinon.stub();
			lbchd._checkReindex = sinon.stub().callsArg(1);
			lbchd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			lbchd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(lbchd, '_spawnChildProcess');
			lbchd._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						lbchd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			lbchd._spawnChildProcess(function(err) {
				...
				lbchd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				lbchd.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn lbchd spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestLBChService = proxyquire('../../lib/services/lbchd', {
				fs: {
			...
			lbchd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						lbchd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			lbchd.spawn.config.rpcpassword = 'password';
			lbchd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			lbchd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			lbchd._spawnChildProcess(function(err) {
				lbchd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			lbchd._initZmqSubSocket = sinon.stub();
			lbchd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			lbchd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var lbchd = new LBChService(config);
			lbchd.node.stopping = true;
			lbchd.startRetryInterval = 100;
			lbchd._loadTipFromNode = sinon.stub();
			lbchd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to lbchd/);
				lbchd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			lbchd.startRetryInterval = 1;
			var config = {};
			lbchd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				lbchd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			lbchd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			lbchd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				lbchd._loadTipFromNode.callCount.should.equal(1);
				lbchd._initZmqSubSocket.callCount.should.equal(1);
				lbchd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.options = {};
			lbchd.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('LBCh configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			lbchd.options = {
				connect: [
					{}
				]
			...
			lbchd.start(function(err) {
				lbchd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var lbchd = new LBChService(baseConfig);
			var node = {};
			lbchd._initChain = sinon.stub().callsArg(0);
			lbchd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			lbchd.options = {
				spawn: {}
			};
			lbchd.start(function(err) {
				should.not.exist(err);
				lbchd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(lbchd, '_spawnChildProcess');
			var nodes = [{}];
			lbchd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			lbchd.options = {
				connect: [
					{}
				]
			...
			lbchd.start(function(err) {
				should.not.exist(err);
				lbchd._initChain.callCount.should.equal(1);
				lbchd._connectProcess.callCount.should.equal(1);
				lbchd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			lbchd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				lbchd.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			lbchd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			lbchd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			lbchd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.syncPercentage(function(err) {
				should.exist(err);
				lbchd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			lbchd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				lbchd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var lbchd = new LBChService(baseConfig);
			var args = lbchd._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var lbchd = new LBChService(baseConfig);
			var args = lbchd._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_normalizeAddressArg');
			lbchd.nodes.push({
				client: {
			...
			var options = {};
			lbchd.getAddressBalance(address, options, function(err) {
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			lbchd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				lbchd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					lbchd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			lbchd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = lbchd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = lbchd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = lbchd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = lbchd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				lbchd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			lbchd.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(lbchd, '_getHeightRangeQuery');
			sinon.spy(lbchd, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			lbchd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				lbchd._getHeightRangeQuery.callCount.should.equal(1);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			lbchd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				lbchd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			lbchd.getAddressTxids(address, options, function(err, txids) {
				...
				lbchd.getAddressTxids(address, options, function(err, txids) {
					...
					lbchd._getHeightRangeQuery.callCount.should.equal(2);
					lbchd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			lbchd.getAddressTxids(address, options, function(err, txids) {
				...
				lbchd.getAddressTxids(address, options, function(err, txids) {
					...
					lbchd._getHeightRangeQuery.callCount.should.equal(4);
					lbchd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			lbchd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				lbchd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					lbchd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						lbchd._getHeightRangeQuery.callCount.should.equal(3);
						lbchd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var lbchd = new LBChService(baseConfig);
			lbchd.height = 10;
			var confirmations = lbchd._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var lbchd = new LBChService(baseConfig);
			var result = {};
			lbchd._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var lbchd = new LBChService(baseConfig);
			var result = {};
			lbchd._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var lbchd = new LBChService(baseConfig);
			var result = {};
			lbchd._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var lbchd = new LBChService(baseConfig);
			var result = {};
			lbchd._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(lbchd, '_getAddressDetailsForInput');
			sinon.spy(lbchd, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = lbchd._getAddressDetailsForTransaction(tx, addresses);
			...
			lbchd._getAddressDetailsForInput.callCount.should.equal(1);
			lbchd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_getConfirmationsDetail');
			lbchd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			lbchd.height = 300;
			var addresses = {};
			lbchd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			lbchd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				lbchd.getDetailedTransaction.callCount.should.equal(1);
				lbchd._getAddressDetailsForTransaction.callCount.should.equal(1);
				lbchd._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var lbchd = new LBChService(baseConfig);
			lbchd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			lbchd._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				lbchd.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from lbchcore addresses', function() {
			var addresses = [
				lbchcore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				lbchcore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var lbchd = new LBChService(baseConfig);
			var strings = lbchd._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var lbchd = new LBChService(baseConfig);
			(function() {
				lbchd._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_normalizeAddressArg');
			lbchd.maxAddressesQuery = 100;
			lbchd.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_normalizeAddressArg');
			sinon.spy(lbchd, '_getAddressStrings');
			lbchd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				lbchd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(lbchd, '_normalizeAddressArg');
			sinon.spy(lbchd, '_getAddressStrings');
			lbchd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			lbchd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				lbchd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(lbchd, '_normalizeAddressArg');
			sinon.spy(lbchd, '_getAddressStrings');
			lbchd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			lbchd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			lbchd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				lbchd._getAddressStrings.callCount.should.equal(1);
				lbchd.getAddressTxids.callCount.should.equal(1);
				lbchd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(lbchd, '_normalizeAddressArg');
			sinon.spy(lbchd, '_getAddressStrings');
			sinon.spy(lbchd, 'getAddressTxids');
			lbchd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			lbchd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				lbchd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(lbchd, '_normalizeAddressArg');
			sinon.spy(lbchd, '_getAddressStrings');
			sinon.spy(lbchd, '_paginateTxids');
			lbchd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(lbchd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			lbchd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			lbchd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				lbchd._getAddressStrings.callCount.should.equal(1);
				lbchd.getAddressTxids.callCount.should.equal(1);
				lbchd._getAddressDetailedTransaction.callCount.should.equal(2);
				lbchd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			lbchd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			lbchd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			lbchd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			lbchd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			lbchd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			lbchd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			lbchd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			lbchd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			lbchd.getAddressSummary(address, options, function(err, summary) {
				lbchd._paginateTxids.callCount.should.equal(1);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				lbchd.getAddressTxids.callCount.should.equal(1);
				lbchd.getAddressBalance.callCount.should.equal(1);
				lbchd._paginateTxids.args[0][1].should.equal(0);
				lbchd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			lbchd.getAddressSummary(address, options, function(err, summary) {
				...
				lbchd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					lbchd._paginateTxids.callCount.should.equal(0);
					lbchd._normalizeAddressArg.callCount.should.equal(2);
					lbchd.getAddressTxids.callCount.should.equal(1);
					lbchd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			lbchd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				lbchd._paginateTxids.callCount.should.equal(1);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				lbchd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			lbchd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			lbchd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(lbchd, '_tryAllClients');
			lbchd.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			lbchd.getRawBlock(blockhash, function(err) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			lbchd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			lbchd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			lbchd.getBlock(0, function(err) {
				lbchd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as lbchcore object from height', function(done) {
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			lbchd.getBlock(0, function(err, block) {
				should.not.exist(err);
				lbchd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(lbchcore.Block);
				done();
			});
		});
		it('will getblock as lbchcore object', function(done) {
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			lbchd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			lbchd.getBlock(hash, function(err, block) {
				should.not.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			lbchd.getBlock(0, function(err, block) {
				...
				lbchd.getBlock(0, function(err, block) {
					should.not.exist(err);
					lbchd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			lbchd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			lbchd.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			lbchd.getBlockHeader(10, function(err) {
				lbchd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			lbchd.getBlockHeader(blockhash, function(err) {
				lbchd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			lbchd.getBlockHeader(0, function(err) {
				lbchd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			lbchd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				lbchd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			lbchd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				lbchd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			lbchd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				lbchd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			lbchd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			lbchd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_maybeGetBlockHash');
			lbchd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			lbchd.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				lbchd._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			lbchd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				lbchd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					lbchd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			lbchd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = lbchcore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			lbchd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			lbchd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			lbchd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = lbchcore.Transaction();
			(function() {
				lbchd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getRawTransaction('txid', function(err) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			lbchd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getTransaction('txid', function(err) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			lbchd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				lbchd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(lbchcore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			lbchd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				lbchd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			lbchd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			lbchd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			lbchd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.node.getNetworkName = sinon.stub().returns('testnet');
			...
			lbchd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			lbchd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			lbchd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				lbchd.getSubsidy.callCount.should.equal(1);
				lbchd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			lbchd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.spawn = {};
			lbchd.spawn.process = new EventEmitter();
			lbchd.spawn.process.kill = sinon.stub();
			lbchd.stop(done);
			lbchd.spawn.process.kill.callCount.should.equal(1);
			lbchd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			lbchd.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			lbchd.spawn.process.kill.callCount.should.equal(1);
			lbchd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			lbchd.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var lbchd = new LBChService(baseConfig);
			lbchd.shutdownTimeout = 300;
			lbchd.spawn = {};
			lbchd.spawn.process = new EventEmitter();
			lbchd.spawn.process.kill = sinon.stub();
			lbchd.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_normalizeAddressArg');
			sinon.spy(lbchd, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			lbchd.nodes.push({
			...
			lbchd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				lbchd._getBalanceFromMempool.callCount.should.equal(1);
				lbchd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			lbchd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			lbchd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			lbchd.getJsonBlock(0, function(err) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			lbchd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			lbchd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				lbchd._tryAllClients.callCount.should.equal(2);
				lbchd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			lbchd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
				lbchd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var lbchd = new LBChService(baseConfig);
			var emitter = new EventEmitter();
			lbchd.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(lbchd.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var lbchd = new LBChService(baseConfig);
			var emitter = new EventEmitter();
			lbchd.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			lbchd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			lbchd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			lbchd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.unsubscribeBalance(emitter1);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			lbchd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var lbchd = new LBChService(baseConfig);
			...
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			lbchd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var lbchd = new LBChService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(lbchd, 'unsubscribeBalanceAll');
			lbchd.unsubscribeBalance(emitter1);
			lbchd.unsubscribeBalanceAll.callCount.should.equal(1);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			lbchd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			lbchd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			lbchd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			lbchd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			lbchd.unsubscribeBalanceAll(emitter1);
			lbchd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			lbchd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			lbchd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(lbchd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var lbchd = new LBChService(baseConfig);
			sinon.spy(lbchd, '_notifyBalanceSubscriber');
			lbchd._getAddressesFromTransaction = sinon.stub().returns(addresses);
			lbchd.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				lbchd._notifyBalanceSubscriber.callCount.should.equal(1);
				lbchd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			lbchd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				lbchd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			lbchd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			lbchd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getDgpInfo(function(err, result) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getMiningInfo(function(err, result) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			lbchd.getStakingInfo(function(err, result) {
				should.exist(err);
				lbchd._tryAllClients.callCount.should.equal(1);
