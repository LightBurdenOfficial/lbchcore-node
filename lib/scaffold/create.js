...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'lbchcore-lib': 'https://github.com/LightBurdenOfficial/lbchcore-lib.git#master',
    'lbchcore-node': 'https://github.com/LightBurdenOfficial/lbchcore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/lbchcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the lbchcore-node directory and configuration
