...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for lbchd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and lbchcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed lbchcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './lbchcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in lbchcore-node services directory
    ...
    if (servicePackage.lbchcoreNode) {
      serviceModule = service.name + '/' + servicePackage.lbchcoreNode;
    }
