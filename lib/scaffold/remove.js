...
/**
 * Will remove a service from lbchcore-node.json
...
/**
 * Will remove the Node.js service and from the lbchcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The lbchcore-node configuration directory
...
function remove(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'lbchcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a lbchcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to lbchcore-node.json
