...
var bitcore = require('lbchcore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var LBChRPC = require('lbchd-rpc');
...
/**
 * Provides a friendly event driven API to lbchd in Node.js. Manages starting and
 * stopping lbchd as a child process for application support, as well as connecting
 * to multiple lbchd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function LBCh(options) {
  if (!(this instanceof LBCh)) {
    return new LBCh(options);
  }
...
util.inherits(LBCh, Service);

LBCh.dependencies = [];

LBCh.DEFAULT_MAX_TXIDS = 1000;
LBCh.DEFAULT_MAX_HISTORY = 50;
LBCh.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
LBCh.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
LBCh.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
LBCh.DEFAULT_SPAWN_RESTART_TIME = 5000;
LBCh.DEFAULT_SPAWN_STOP_TIME = 10000;
LBCh.DEFAULT_TRY_ALL_INTERVAL = 1000;
LBCh.DEFAULT_REINDEX_INTERVAL = 10000;
LBCh.DEFAULT_START_RETRY_INTERVAL = 5000;
LBCh.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
LBCh.DEFAULT_TRANSACTION_CONCURRENCY = 5;
LBCh.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
LBCh.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || LBCh.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || LBCh.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || LBCh.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || LBCh.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || LBCh.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || LBCh.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || LBCh.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || LBCh.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || LBCh.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || LBCh.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

LBCh.prototype._initCaches = function() {
...
LBCh.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
LBCh.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
LBCh.prototype.getPublishEvents = function() {
  return [
    {
...
LBCh.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'lbchd/' + name, 'total:', this.subscriptions[name].length);
};

LBCh.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'lbchd/' + name, 'total:', this.subscriptions[name].length);
};


LBCh.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
LBCh.prototype.unsubscribeBalance = function(emitter, addresses) {
...
LBCh.prototype.unsubscribeBalanceAll = function(emitter) {
...
LBCh.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'lbchd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

LBCh.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'lbchd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
LBCh.prototype.unsubscribeAddressAll = function(emitter) {
...
LBCh.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = LBCh.DEFAULT_CONFIG_SETTINGS;
...
LBCh.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
LBCh.prototype._expandRelativeDatadir = function() {
...
LBCh.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in lbchd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in lbchd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in lbchd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './lbch.conf');

  log.info('Using lbch config file:', configPath);
...
LBCh.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of lbchcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of lbchcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of lbchcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to lbchd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from lbchd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from lbchd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that lbchd is undergoing a reindex. ' +
             ...
             'of lbchcore-node services will start.');
    node._reindex = true;
  }
};

LBCh.prototype._resetCaches = function() {
...
LBCh.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
LBCh.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

LBCh.prototype._initChain = function(callback) {
  var self = this;
...
LBCh.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
LBCh.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/lbch.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/lbch.conf';
    }
  }
  return networkPath;
};

LBCh.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
LBCh.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('lbchd/hashblock', message.toString('hex'));
    }
  }

};

LBCh.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
LBCh.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('LBCh Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
LBCh.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
LBCh.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
LBCh.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
LBCh.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('lbchd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
LBCh.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('lbchd/rawtransaction', message.toString('hex'));
    }
...
LBCh.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || LBCh.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

LBCh.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
LBCh.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
LBCh.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('LBCh Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || LBCh.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

LBCh.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
LBCh.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/lbchd.pid';
...
LBCh.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting lbch process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('LBCh process unexpectedly exited with code:', code);
        log.warn('Restarting lbch child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('LBCh process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn lbchd.'));
      }
...
LBCh.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to lbchd.'));
    }
 ...
 */
LBCh.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('LBCh configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
LBCh.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
LBCh.prototype.syncPercentage = function(callback) {
  var self = this;
...
LBCh.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, lbchcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
LBCh.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
LBCh.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
LBCh.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
LBCh.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in lbchcore is most recent to less recent
 ...
 * @param {Function} callback
 */
LBCh.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
LBCh.prototype._getConfirmationsDetail = function(transaction) {
...
LBCh.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
LBCh.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
LBCh.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A LBCH transaction id
 * @param {Function} callback
 */
LBCh.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
LBCh.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
LBCh.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
LBCh.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
LBCh.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
LBCh.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
LBCh.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
LBCh.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
LBCh.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
LBCh.prototype.estimateSmartFee = function (blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a LBChcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
LBCh.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getInfo = function(callback) {
...
LBCh.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('lbchd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('lbchd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
LBCh.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
LBCh.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
LBCh.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
LBCh.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
LBCh.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
LBCh.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
LBCh.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
LBCh.prototype.getStakingInfo = function(callback) {
    var self = this,
